...
class VocabLessonAnswerRequest(BaseModel):
    answer: Any
...
def answer_question(
    lesson_id: int,
    question_id: int,
    payload: VocabLessonAnswerRequest,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Per-question immediate feedback (Vocab reveals correctness one
    question at a time, unlike Reading). Quota is checked but NOT logged
    here — only /check (the full-lesson submission) consumes the cap."""
    vocab_lesson_quota.check_quota(user, db)

    question = vocab_lesson_service.get_question(lesson_id, question_id)
    if question is None:
